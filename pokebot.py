...
def find_pokemon_id(name):
    ...
    if name == 'Nidoran-F':# -*- coding: utf-8 -*-
import random
import asyncio
import discord
from discord.ext import commands
        ...
        if random.randint(1,200) < 197:
            ...
            if client.wait_for_message(author=None, content="$throw"):
                ...
                if throw_rate <= 30:
                    ...
                    await asyncio.sleep(1)
                    await client.purge_from(message.channel, limit=1, check=is_pokebot)
                    await client.send_message(message.channel, "great throw!")
                    await asyncio.sleep(1.5)
                    await client.purge_from(message.channel, limit=1, check=is_pokebot)
                    await client.send_file(message.channel, "images/shake.png")
                    await asyncio.sleep(1.5)
                    ...
                    await asyncio.sleep(1)
                    await client.purge_from(message.channel, limit=1, check=is_pokebot)
                    await client.send_message(message.channel, "nice throw!")
                    await asyncio.sleep(1.5)
                    await client.purge_from(message.channel, limit=1, check=is_pokebot)
                    await client.send_file(message.channel, "images/shake.png")
                    await asyncio.sleep(1.5)
            ...
            async def spawn_shiny_pokes():
                embed_shiny_spawn = discord.Embed(
                    ...
                    description="CP: " + str(cp) + "\nTo catch the wild pokemon, type $throw", color = 3447003)
                embed_shiny_spawn.set_thumbnail(url="https://img.pokemondb.net/sprites/omega-ruby-alpha-sapphire/dex/shiny/" + str(pokechoice).lower() + ".png")
            ...
            if client.wait_for_message(author=None, content="$throw"):
                ...
                async def shiny_caught():
                    ...
                    embed_shiny_caught.set_thumbnail(url="https://img.pokemondb.net/sprites/omega-ruby-alpha-sapphire/dex/shiny/" + str(pokechoice).lower() + ".png")
                ...
                async def shiny_fled():
                    ...
                    embed_shiny_fled.set_thumbnail(url="https://img.pokemondb.net/sprites/omega-ruby-alpha-sapphire/dex/shiny/" + str(pokechoice).lower() + ".png")
...
client.run("MzY2ODU0MjUyMDk5OTI4MDY0.DLy7MA.IeVjeCSjdgc-IUgm2nsZPL_OYRI")
        return 29
    elif name == 'Nidoran-M':
        return 32
    elif name == 'Mr-Mime':
...
async def on_message(message):
    ...
    if message.content.startswith("$raid"):
        ...
        if client.wait_for_message(author=message.author, content="$raid"):
            ...
            await client.send_message(message.channel, "To attack, type `$attack`")
            await asyncio.sleep(0.1)
            #make sure $attack is written before everything else begings
            def check(msg):
                return msg.content.startswith("$attack")
            def leg_catch(msg):
                return msg.content.startswith("$catch")
...
client.run("INSERT BOT TOKEN BETWEEN THESE QUOTES")
